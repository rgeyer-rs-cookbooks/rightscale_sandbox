...
#        source gem_source
#      end
#      gem_binary node["rightscale_sandbox"]["gem_bin"]
#      action :nothing
#    end
#
#    g.run_action(:install) if ::File.exists? node["rightscale_sandbox"]["gem_bin"]

    gem_already_installed = `#{node["rightscale_sandbox"]["gem_bin"]} list | findstr "#{gem_name}"`
    ...
    if gem_already_installed.strip == ""
      ...
      `#{node["rightscale_sandbox"]["gem_bin"]} #{install_params}`
    end
  else
